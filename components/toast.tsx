...
import {
  ...
  choose_toast_status,
  choose_toast_time_out,
  delete_toast_message,
  delete_toast_status,
  delete_toast_time_out,
} from "../root/commonSlice";

function GetToast({ status, message, timeOut }: any) {
  ...
  return (
    <div className={`toast111 ${incoming ? "open" : "close"}`}>
      {(status1 === 1 && (
        <div className="flex items-center bg-green-500 border-l-4 border-green-700 py-2 px-3 shadow-md mb-2">
          ...
          <div className="text-white max-w-xs ">{message1}</div>
        </div>
      )) ||
        (status1 === 0 && (
...
export default function Toast() {
  ...
  useEffect(() => {
    if (toast_message) {
      ...
      let newListToast: any = listToast;
